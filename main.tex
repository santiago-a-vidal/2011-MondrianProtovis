...
\section{Protovis}
Protovis\footnote{\url{http://vis.stanford.edu/protovis}} \TG{add the reference to the Protovis paper} is a visualization toolkit written in JavaScript that enables the creation of complex and interactive graphics in web-browsers without the need of an extra plugin. The data to be visualized is defined by means of simple marks such as bars, lines or dots. 

Protovis makes intensive use of a fluent interface. For example, to define a bar of \ct{height=100} and \ct{width=20} (Figure \ref{fig:SimpleBar}) with a label the following code must be used:
...
\TG{Mondrian Easel is just an editor tool. You generate a view with Mondrian, which is the engine.}

Given a view generated with Mondrian \cite{Meye06a} we want to be able to export it to an HTML file using the Protovis toolkit. In addition of obtaining a similar visualization graph, interaction with it (such as drag and drop) is required. 

\TG{This is not useful}
The general scenario of exportation is: 
\begin{enumerate}
\item A user generates a view using Mondrian
\item A user chooses the option "Export as HTML"
\item A file that re-create the view found in Mondrian is generated using Protovis. 
\end{enumerate}

\sv{What are the main advantages of this kind of visualization. What are the contributions??} \TG{The only contribution is that it is available on the browser. It is not scientific, just engineering. However, we can talk about the challenges of transforming the model of Mondrian in the model of Protovis. For example, we can talk about the problem of arranging the nodes / edges in layers, the scoping of drag and drop}
...
The standard way to send data to Protovis is by means of a JSON (JavaScript Object Notation) descriptions. Owing to the components to be exported are currently drawn in the Mondrian canvas, the dataset description must be inferred from it. As the canvas data is represented by mean of the \ct{MOGraphElement} hierarchy, the dataset description is created through a Visitor pattern \TG{[REF GAMMA]} as is shown in Figure \ref{fig:VisitorPattern}.
...
\end{figure}
The JSON structure that we generate is created from the nodes and edges drawn on the canvas. For this reason the structure contains two arrays: nodes and links. The former contain a collection of objects that represent nodes by means of information that allows its correct visualization with Protovis. For example, given the next JSON object a node is specified
\begin{lstlisting} 
{
...
As Protovis uses a declarative syntax each kind of mark is declared once \TG{What does ``declared once'' mean? It would be declared once no matter what :)}. This is different only for lines which are declared individually for each link of the dataset. In this way to visualize nodes whose shape is \ct{MORectangleShape} the next code must be declared once
\begin{lstlisting} 
vis.add(pv.Bar)
