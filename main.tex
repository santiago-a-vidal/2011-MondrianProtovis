...
The standard way to send data to Protovis is by means of a JSON (JavaScript Object Notation) descriptions. Owing to the components to be exported are currently drawn in the Mondrian canvas, the dataset description must be inferred from it\ab{I do not understand this sentence}\sv{I meant that we are going to obtain the data from the canvas of Mondrian (The visitor gets the data from the canvas)}. As the canvas data is represented by mean of the \ct{MOGraphElement} hierarchy, the dataset description is created through a Visitor pattern~\cite{Gamm95a} as shown in Figure \ref{fig:VisitorPattern}.
...
\end{figure}
The JSON structure that we generate is created from the nodes and edges composing the Mondrian view. For this reason the structure \ab{Which structure? The structure generated by our exporter written in JSON?} \sv{right. The dataset that our exporter generates in JSON is composed by 2 arrays. That is: dataset=[nodesArray,linksArray]} contains two arrays: nodes and links. The former contain a collection of objects that represent nodes by means of information that allows its correct visualization with Protovis. For example, given the next JSON object a node is specified
\begin{lstlisting} 
{
